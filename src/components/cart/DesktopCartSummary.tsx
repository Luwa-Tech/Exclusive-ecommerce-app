...
const DesktopCartSummary = (props: CartSummaryType) => {
    return (
        <div className="px-[.7rem] py-[.4rem] md:h-[11rem] border-[.1rem]">
            ...
            <button onClick={() => props.checkoutHandler(props.userCart)} className="bg-secondary-700 text-textColor-400 px-[.5rem] py-[.4rem] rounded-[.2rem] uppercase mt-2 w-[100%] hover:opacity-[0.6]">Checkout {formatCurrency(props.totalPrice)}</button>
        </div>
    )
}
